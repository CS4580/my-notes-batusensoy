...
import pandas as pd
import numpy as np
import get_data as gt # my package
...
def knn_analysis_driver(data_df, base_case, comparison_type, metric_func, sorted_value='metric'):
    ...
    # Print top K values
    print_top_k(sorted_df, sorted_value, comparison_type) # default is ascending = True
...
def jaccard_similarity_normal(base_case_genres: str, comparotar_genres: str):
    base_case_genres = set(base_case_genres.split(';'))
    comparotar_genres = set(comparotar_genres.split(';'))
    numerator = len(base_case_genres.intersection(comparotar_genres))
    denominator = len(base_case_genres.union(comparotar_genres))
